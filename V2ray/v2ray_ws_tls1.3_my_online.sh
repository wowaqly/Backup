...
function install_v2ray(){
    ...
    rm -f config.json
    cp /root/v2ray-config.json /v2ray/config.json >/dev/null 2>&1
...
[Service]
Type=simple
ExecStart=/v2ray/v2ray -config /v2ray/config.json 
Restart=on-abort
...
function update_v2ray(){
	...
	cp /v2ray/config.json /root/lingshi389
	rm -rf /v2ray
    mkdir /v2ray
    cp /root/v2ray-linux-64.zip /v2ray
	cd /v2ray
	...
	rm -f v2ray-linux-64.zip
    rm -f config.json
	cp /root/lingshi389/config.json /v2ray
